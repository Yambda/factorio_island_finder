...
Each image was generated by independently re-reading the actual source file
it depicts (not assumed from memory) before rendering, and each is labeled
with its exact parameters, so you can check the claims against the code
yourself:

- `1_mesh_terrain.png` — the raw terrain (mesh) output around spawn, no
  island highlighted — this is what the pipeline sees before any stage
  does anything with it. Water in blue-gray, land in red-brown.
- `2_cascade_pass0_128tile.png` — pass 0, the coarsest cascade tier
  (`--stage15-three-tier`'s addition): 128-tile net spacing over the full
  real domain (domain_radius_nodes=56, ±7,168 tiles), with the
  `RING_TILES=5000` boundary drawn and the confirmed island highlighted
  in green.
...
- `5_floodfill_result.png` — the real, exact flood-fill result
  (reimplementing `code/floodfill/stages.cpp`'s own algorithm): island in
  green, connected water in blue-gray, mainland in red.
